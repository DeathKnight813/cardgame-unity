namespace Game.Utils
{
    ...
    public class Utility
    {
        ...
        /// <returns></returns>
        public static string troymd5(string str, int code)
        {
            if (code == 32)
            {
                string Encrypt = System.Web.Security.FormsAuthentication.HashPasswordForStoringInConfigFile(str, "MD5").ToLower();
                for (int i = 30; 0 <= i; i = i - 2)
                {
                    if (Encrypt.Substring(i, 1) == "0")
                    {
                        Encrypt = Encrypt.Remove(i, 1);
                    }
                }
                return Encrypt;
            }
