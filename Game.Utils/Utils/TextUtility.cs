namespace Game.Utils
{
    ...
    public class TextUtility
    {
        ...
        #endregion
        //wolf3
        public static bool IsValidSQLStr(string text)
        {
            string strInput = text.ToLower();
            string[] blackList = { "--", ";--", ";", "/*", "*/", "@@", "'", "%", "xp_", "sp_", "=", "char", "alter", "begin", "cast", "create", "cursor", "declare", "delete", "drop",  "end", "exec", 
                                     "fetch", "insert", "information_schema", "kill", "left","like", "limit", "lpad", "right", "rpad", "select", "substr", "shutdown", "sysobjects", "syscolumns", 
                                     "where", "table", "update"};/* "#", "@", "open", "sys"*/
            for (int i = 0; i < blackList.Length; i++)
            {
                if (strInput.Contains(blackList[i]))
                {
                    return false;
                }
            }
